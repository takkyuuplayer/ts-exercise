...
describe("typesafe-actions", () => {
  ...
  let id = 0;
  const actions = {
    addTodo: createAction(TodoActionTypes.addTodo, (title: string) => {
      id++;
  ...
  describe("action", () => {
    ...
    it("with payload and logic", () => {
      expect(actions.addTodo("Learn TypeScript")).toEqual({
        payload: {
          completed: false,
          id: 1,
  ...
  describe("reducer", () => {
    ...
    it("handles addTodo", () => {
      expect(
        reducer(undefined, actions.addTodo("Learn TypeScript")).todos
